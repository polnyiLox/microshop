import logging

from redis.asyncio import Redis
from redis.exceptions import RedisError

from app.cache.base import Cache
from app.core.config import RedisSettings


logger = logging.getLogger(__name__)


class RedisCache(Cache):
    def __init__(self, settings: RedisSettings) -> None:
        self._settings = settings
        self._client: Redis | None = None

    async def connect(self) -> None:
        if self._client is not None:
            return
        client = Redis.from_url(
            self._settings.url,
            decode_responses=True,
        )
        try:
            await client.ping()
        except (RedisError, OSError):
            await client.aclose()
            logger.warning(
                "Redis unavailable; payment-service will use PostgreSQL",
                exc_info=True,
            )
        else:
            self._client = client
            logger.info("Payment-service connected to Redis")

    async def close(self) -> None:
        if self._client is not None:
            await self._client.aclose()
            self._client = None
            logger.info("Payment-service Redis connection closed")

    async def get(self, key: str) -> str | None:
        if self._client is None:
            return None
        try:
            return await self._client.get(self._key(key))
        except (RedisError, OSError):
            logger.warning("Failed to get data from Redis", exc_info=True)
            return None

    async def set(self, key: str, value: str, ttl_seconds: int) -> None:
        if self._client is None:
            return
        try:
            await self._client.set(self._key(key), value, ex=ttl_seconds)
        except (RedisError, OSError):
            logger.warning("Failed to set data in Redis", exc_info=True)

    async def delete(self, *keys: str) -> None:
        if self._client is None or not keys:
            return
        try:
            await self._client.delete(*(self._key(key) for key in keys))
        except (RedisError, OSError):
            logger.warning("Failed to delete data from Redis", exc_info=True)

    def _key(self, key: str) -> str:
        return f"{self._settings.key_prefix}:{key}"

    def create_payment_key(self, payment_id: str) -> str:
        return f"{self._settings.payment_key}{payment_id}"

    def create_order_payments_key(self, order_id: str) -> str:
        return f"{self._settings.order_payments_key}{order_id}"

    def create_user_payments_key(self, user_id: str) -> str:
        return f"{self._settings.user_payments_key}{user_id}"
